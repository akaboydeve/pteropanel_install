...
#############################################################################
#                                                                           #
# Project 'pterodactyl-installer' for wings SIMPLE INSTALL                  #
#                                                                           #
...
ask_database_user() {
  echo -n "* automatically configure a user for database hosts"
  CONFIRM_DBHOST=1

  if [[ "$CONFIRM_DBHOST" =~ [1] ]]; then
...
ask_database_external() {
  echo -n "* configure MySQL to be accessed externally"
  CONFIRM_DBEXTERNAL=1

  if [[ "$CONFIRM_DBEXTERNAL" =~ [1] ]]; then
    echo -n "* Enter the panel address (blank for any address PRESS ENTER): "
...
ask_database_firewall() {
  print_warning "Allow incoming traffic to port 3306 (MySQL) can potentially be a security risk"
  echo -n "* allow incoming traffic to port 3306"
  CONFIRM_DB_FIREWALL=1
  if [[ "$CONFIRM_DB_FIREWALL" =~ [1] ]]; then
...
function goodbye {
  echo ""
  print_brake 70
  echo "* SIMPLE Wings installation completed"
  echo "*"
